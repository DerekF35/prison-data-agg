...
A standardized, deduplicated, and verified national database and multi-format spreadsheets (CSV & Excel) of all **6,768 physical correctional facilities** in the United States, spanning federal, state, county, municipal, and private sectors across all 50 states, the District of Columbia, and 5 US territories.
...
| **Master CSV Dataset** | [`output/us_correctional_facilities_master.csv`](file:///home/derekf35/Development/PROJECTS/prison-data-agg/output/us_correctional_facilities_master.csv) | UTF-8 CSV, 6,768 rows, 20 standardized columns with preserved leading zeroes |
...
* **Geocoded Facilities (Mapped GPS)**: 6,768 (100.0%)
* **Facilities with Street Addresses**: 6,765 (100.0%)
* **Facilities with Telephone Numbers**: 6,228 (92.0%)
...
2. **Type-Guarded BOP Entity Matching**:
   * To prevent false positives where short 3-letter BOP codes (e.g. `LOS` for MDC Los Angeles or `FOR` for FCI Forrest City) match county jail names (e.g. *Los Angeles County Sybil Brand Institute*), the pipeline enforces an explicit Federal / Multi-jurisdiction type check before applying abbreviation or title matching.
...
3. **Preservation of Legitimate Zero Counts**:
   * `clean_int()` preserves valid `0` counts for unoccupied, newly constructed, or temporary intake facilities, while scrubbing negative placeholder sentinels (`-999`, `-1`, `99999`) to clean nulls.
...
===========================================================================
[PASS] CSV deliverable verified (1,880,415 bytes)
[PASS] Excel deliverable verified (1,034,373 bytes)
[PASS] JSON deliverable verified (1,980 bytes)
[PASS] Word deliverable verified (40,742 bytes)
[PASS] PDF deliverable verified (100,421 bytes)
[PASS] ZIP deliverable verified (1,548,220 bytes)

[INFO] Total Records Loaded: 6,768
...
[PASS] Test 6: 100.0% County & 5-digit FIPS code completeness across all 6,768 records
[PASS] Test 7: 6,228 phone numbers strictly match formatted canonical regex
[PASS] Test 8: Capacities and Populations stored as clean discrete integers in CSV
...
[PASS] Test 11: All 31 standalone BOP records possess 100.0% valid GPS coordinates
[PASS] Test 12: Accounted for 6 campus co-located records and 4 co-located agency offices
[PASS] Test 13: Exact 1-to-1 parity between CSV and Excel Master Directory (6,768 rows, 20 columns)
[PASS] Test 14: Excel Summary Tabs match ground truth sums perfectly (6,768 facilities, 2,411,708 capacity, 2,069,547 population)
[PASS] Test 15: Excel Data Dictionary features full 3-column Display Header to CSV snake_case key mapping

===========================================================================
ALL 15 ADVERSARIAL FORENSIC AUDIT TESTS PASSED WITH ZERO DEFECTS!
===========================================================================
```
